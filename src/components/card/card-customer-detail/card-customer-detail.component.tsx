...
export function CardCustomerDetail({ tag, name, address, image, logo }: ICardCustomerDetailProps) {
    return (
        <div className='relative flex flex-col gap-2 cursor-pointer'>
            {tag ? <BadgeCustomerTag className='absolute left-2 top-2 md:left-5 md:top-5 shadow-lg' name={tag.name} icon={tag.icon}/>: false}
            <div className='w-full h-[150px] md:h-[250px] rounded-2xl overflow-hidden'
                style={{
